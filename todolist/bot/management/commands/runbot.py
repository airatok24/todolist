...
class Command(BaseCommand):
    ...
    def handle_goal_categories_list(self, msg: Message, tg_user: TgUser):
        resp_categories: list[str] = [
            ...
            for cat in GoalCategory.objects.filter(
                board__participants__user__id=tg_user.user_id,
                is_deleted=False
            )
        ]
        if resp_categories:
    ...
    def handle_save_selected_category(self, msg: Message, tg_user: TgUser):
        if msg.text.isdigit():
            ...
            if GoalCategory.objects.filter(
                    ...
                    board_participants__role__in=[BoardParticipant.Role.owner, BoardParticipant.Role.writer],
                    is_deleted=False,
                    id=cat_id
            ).exists():
                self.storage.update_data(chat_id=msg.chat.id, cat_id=cat_id)
                self.tg_client.send_message(msg.chat.id, '[set_title]')
    ...
    def handle_verified_user(self, msg: Message, tg_user: TgUser):
        if msg.text == '/goals':
            ...
            self.tg_client.send_message(msg.chat.id, '[unknown_command]')
